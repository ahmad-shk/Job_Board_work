...
function EmailVerified() {
  ...
  const emailVarification = async () => {
    try {
      setLoading(true)
      const res = await putCall(`/auth/verify-email/${queryParams.email}`)
      const { data } = res
      if (data.success) {
    ...
    <div className="flex items-center justify-center h-screen bg-white">
      <div className="text-center">
        <div className="flex flex-col items-center justify-center">
          <img src="/assets/images/Emailverified.svg" alt="" className="h-[180px] w-[180px]" />
          ...
          <div className="p-3">
            <button className="inline-flex py-2 px-4 rounded bg-yellow-500 hover:bg-yellow-400 transition-colors duration-100" onClick={() => { emailVarification() }}>              
