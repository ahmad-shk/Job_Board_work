...
const JobSearchListing = () => {
    ...
    useEffect(() => { dispatch(analyticsData()) }, [])
    const getPaginatedJobs = () => { return jobs?.slice(offset, offset + PER_PAGE); };
    const modalOpen = (item) => { setModal(true); handleClick(jobs?.indexOf(item)) }
    ...
    const checkprofiileCompletion = () => {
        if (educationTraining && personalInformation && portfolio && skills && work && questions) {
            const count = educationTraining?.completedWeight + personalInformation?.completedWeight + portfolio?.completedWeight + skills?.completedWeight + work?.completedWeight + questions?.completedWeight
            if (count === 100) {
    ...
    const handleApply = (jobId) => {
        setLoading(true)
        dispatch(postApplyJob({ jobId })).unwrap().then((res) => {
            setLoading(false)
            if (res.status === 200) {
                setModal(false)
    ...
    useEffect(() => { if (applyFilter) { getJobData(filter); setApplyFilter(false) } if (filter?.title === '' && filter?.location === '') { setCurrentPage(0) } }, [applyFilter]);
    ...
    const getJobData = async (filterObj) => {
        try {
            ...
            const response = await getCall(`/jobs`, { ...filterObjectWithValue(filterObj), page: (currentPage + 1) });
        ...
        <>
...

            {
                ...
                <div className="rounded-[10px] ">
                    {<JobSearchFilter filter={filter} setFilter={setFilter} setApplyFilter={setApplyFilter} jobs={jobs} getJobData={getJobData} jobCompanyName={jobCompanyName} />}
                    ...
                    <div className=" card bg-[white]  px-[40px] p-2 rounded-[10px]">
                        ...
                        <div className="job-search-table-box z-[10] overflow-x-auto mt-[20px]  p-[5px] scrollnone -pr-[100px]  -m-auto">

                            {
                                ...
                                jobs && jobs?.length > 0 ? <table className="table w-full text-[12px]">
                                    <thead>
                                            ...
                                            jobs && jobs?.length > 0 && jobs?.map((item, index) => {
                                                return (
                                                    <>
                                                        ...
                                                        <tr className=" border-0 cursor-pointer rounded-lg  under-shadow -bg-[red] hover:scale-[1.005]  h-[50px] " id={index}>

                                                            <td className="  rounded-l-lg min-w-[75px] px-[5px] w-[60px] max-w-[75px] p-[0px] min-h-[50px] -bg-[#FFCB05]" onClick={() => { modalOpen(item) }}
                                                            >

                                                                {item?.photo ?
                                                                    <>
                                                                        <img
                                                                            src={item?.photo || "/assets/images/employer-images/legitimacylogo.png"}
                                                                            alt="Profile"
                                                            ...
                                                            <td className="left-shadow  text-[15px] font-bold bg-white text-nowrap " onClick={() => { modalOpen(item) }}>
                                                                {item && item?.businessName ? item?.businessName : "-"}
                                                                <p className="text-[11px] text-nowrap text-[#919191] font-[400]">{item?.createdAt ? moment(item?.createdAt).format('MMMM D, YYYY h:mm A') : "---"}</p>
                                                            </td>
                                                            <td onClick={() => { modalOpen(item) }} className="text-[15px] text-nowrap -w-[20%] bg-white">
                                                                {item?.title?.length > 20 ? `${item?.title.slice(0, 20)}...` : item?.title}<p className="text-[11px] text-[#919191] mt-[5px]">{`${modeEnum[item?.mode]}, ${jobTyoeEnum[item?.type]}, ${jobExperience[item?.experienceLevel]}`}</p></td>
                                                            <td className="text-[15px] text-nowrap bg-white" onClick={() => { modalOpen(item) }}>{item?.city ? `${item?.city}, ${item?.state}` : "---"}
                                                                <p className='text-[12px] text-nowrap leading-[20px] my-[3px] opacity-[0.6]'>{item?.country ? `${CountryDATAEnum[item?.country]}` : "---"}</p>
                                                            </td>
                                                            <td onClick={() => { modalOpen(item) }} className="text-[15px] text-nowrap -w-[20%] bg-white">
                                                                {getPayDisplay(item)}
                                                                <p className="text-[11px] text-[#919191] text-nowrap mt-[5px]">{`${rateUnitEnum[item?.pay?.pay?.rateUnit]}`}</p></td>
                                                            <td className="text-[15px] bg-white  text-nowrap rounded-r-lg">
                                                                <div className="flex items-center gap-2">
                                                                    {/* {console.log(item?.eligibility?.isEligible,index)} */}
                                                                    {item?.eligibility?.isEligible && profileComplettion ?
                                                                        <button className="inline-flex py-[5px] px-[8px] text-[15px] rounded-[8px] bg-[#FFCB05]"
                                                                        ...
                                                                        :
                                                                        <button className="inline-flex py-[5px] px-[8px] text-[15px] rounded-[8px] bg-[#E2E2E2]"
                                                                            disabled={item?.isAlreadyApplied}
                                                                            onClick={() => {
                                                                                if (applyCheck(item)) { return }
                                                                                setSelectedJobId(item?._id)
                                                        ...
                                                        <tr className="border-0 ">
                                                            <td colSpan={7} className="transparent h-[10px] p-0"></td>
                                                        </tr>
                                                    </>
                                                )
                                            })
            ...
            <JobPreviewMode applyCheck={applyCheck} loading={loading} selectedJobId={selectedJobId} jobs={jobs} modal={modal} setModal={setModal} getIndex={getIndex} setSelectedJobId={setSelectedJobId} handleApply={handleApply} />

            {/* -------------Disqualifiers Model----------------------- */}
            <div className={disqualifierModal ? "" : `hidden`}>
                <Disqualifiers profileStatusCheck={profileStatusCheck} disqualifierModal={disqualifierModal} setDisqualifierModal={setDisqualifierModal} disqualifierData={disqualifierData} />
            </div>
            {/* -------------Disqualifiers Model----------------------- */}
        </>
    )
