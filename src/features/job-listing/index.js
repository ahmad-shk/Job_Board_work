...
import InterviewModel1 from './InterviewModel1';
import InterviewModel2 from './InterviewModel2';
import InterviewModel3 from './InterviewModel3';
import ContactRequest from './ContactRequest';
import moment from 'moment'
import { jobTyoeEnum, modeEnum } from '../jobSearch/jobConst';
import { CountryDATAEnum, ExperienceEnum, usaCitiesEnum, USStatesEnum } from '../../pages/employerProtected/JobConsts';
import JobPreviewModel from './JobPriviewModel';
...
const JobListing = () => {
    ...
    const [status, setStatus] = useState({ status: "", page: 1, limit: 50 })
    const { profilePagesData } = useSelector(state => state.profile)
    const { jobBoardData, isLoading } = useSelector(state => state.jobSearch)
    ...
    // useEffect(() => { setId(profilePagesData?.user?._id) }, []);
    const handlePageClick = ({ selected }) => {
        setCurrentPage(selected)
        setStatus((prev) => ({ ...prev, page: selected }))
    }
    useEffect(() => {
        dispatch(getJobBoardData(status));
    }, [currentPage, status]);

    useEffect(() => {
        if (jobBoardData) {
            setTotalJobs(jobBoardData.totalApplications);
            setTotalPages(jobBoardData.totalPages);
        }
    }, [jobBoardData]);
    ...
    const interviewClicked = (index, selectedItem) => {
        setId(profilePagesData?.user?._id)
        console.log("lololosss", selectedItem?.interviewDetails?.seekerResponse.status)
        const { time } = selectedItem?.interviewDetails?.employerProposal || {};
        setInterviewData(selectedItem);
        if (Array.isArray(time?.proposedTime) && time?.proposedTime.length > 0) {
            setActiveModal(0);
        } if (time?.requestAvailability && Object.keys(time?.requestAvailability).length > 0) {
            setActiveModal(1);
        } if (time?.fixedTime && Object.keys(time?.fixedTime).length > 0) {
            setActiveModal(2);
        }
        setInterviewModel(!interviewModel)
    }
    const interViewResponce = async (payload, res) => {
        const jobId = interviewData?._id;
        const updateStatus = res ? "accepted" : "rejected";
        if(payload.time){
            payload = { ...payload, startTime: payload.time }
            delete payload.time
        }
        const finalResponse = {
            jobId: jobId,
        ...
        try {
            ...
            const response = await postCall('/jobs/response-interview-proposal', finalResponse);
    ...
    const [detailForContact, setdetailForContact] = useState()
    const contactReaquest = (item) => {
        setdetailForContact(item)
    ...
    return (
        <>
            {/* -----------------------1---------------------- */}
            <div className={`${interviewModel && activeModal === 0 ? "" : "hidden"}`}>
                <InterviewModel3 interViewResponce={interViewResponce} interviewData={interviewData} setInterviewModel={setInterviewModel} interviewModel={interviewModel} />
            </div>
            {/* -----------------------2---------------------- */}
            <div className={`${interviewModel && activeModal === 1 ? "" : "hidden"}`}>
                <InterviewModel2 interViewResponce={interViewResponce} interviewData={interviewData} setInterviewModel={setInterviewModel} interviewModel={interviewModel} />
            </div>
            {/* -----------------------3---------------------- */}
            <div className={`${interviewModel && activeModal === 2 ? "" : "hidden"}`}>
                <InterviewModel1 interViewResponce={interViewResponce} interviewData={interviewData} setInterviewModel={setInterviewModel} interviewModel={interviewModel} />
            </div>
            {/* ----------------------------------------------------------------- */}
            {/* -------------------Contact Request------------------------------ */}
            <div className={` ${contactRequest ? 'hidden' : ''}`}>
                <ContactRequest status={status} contactRequest={contactRequest} detailForContact={detailForContact} setContactRequest={setContactRequest} />
            </div>
            {/* -------------------Contact Request------------------------------ */}
            <div className="rounded-[10px]">
                <JobListingFilter status={status} setStatus={setStatus} />
                {
                    (jobBoardData && jobBoardData?.length > 0 || !isLoading) ?
                        <div className="card bg-[] job-listing-table-box p-2 px-[40px] rounded-[10px] bg-[white] scrollnone">
                            <CustomOverlay isLoading={isLoadingData} />
                            <div className=" p-[5px]  relative scrollnone -bg-[white]  -bg-[red] relative ">
                                {jobBoardData && jobBoardData?.jobs?.length > 0 ?
                                    <table className="table  w-[100%] text-[12px] relative">
                                        <thead className="">
                                        ...
                                        <tbody>
                                            {jobBoardData && jobBoardData?.jobs?.length > 0 && jobBoardData?.jobs?.map((item, index) => (
                                                <>
                                                    <tr className=" border-0 cursor-pointer rounded-lg under-shadow -bg-[white] h-[50px] " >
                                                        <td className="-border-r rounded-l-lg min-w-[75px] px-[5px] max-w-[75px] p-[0px] min-h-[50px] -bg-[#FFCB05]" onClick={() => { modalOpen(item) }}
                                                        // style={{ backgroundImage: `url(${item?.photo})`, backgroundPosition: 'center', backgroundSize: 'contain', backgroundRepeat: 'no-repeat' }}
                                                        >

                                                            {/* ----------------------------------------------------------------------------------- */}
                                                            {item?.photo ? <>
                                                                <img
                                                                    src={item?.photo || "/assets/images/applicant-tab-profile.svg"}
                                                                    alt="Profile"
                                                        ...
                                                        <td onClick={() => { setCurrentItem(item); setModal(true) }} className=" left-shadow text-nowrap w-[22%] text-[15px] font-bold cursor-pointer bg-white">{item?.businessName ? item?.businessName : "---"}<p className="text-[11px] text-[#919191] mt-[5px] whitespace-nowrap">{item?.appliedAt ? moment(item?.appliedAt).format('MMMM D, YYYY h:mm A') : "---"}</p></td>
                                                        <td onClick={() => { setCurrentItem(item); setModal(true) }} className="text-nowrap w-[22%] text-[15px] bg-white">{item?.title.length > 25 ? `${item?.title.slice(0, 25)}...` : item?.title}<p className="text-[11px] text-[#919191] mt-[5px]">{`${modeEnum[item?.mode]}, ${jobTyoeEnum[item?.type]}, ${ExperienceEnum[item?.experienceLevel]}`} </p></td>
                                                        <td onClick={() => { setCurrentItem(item); setModal(true) }} className="text-nowrap text-[15px] w-[22%] bg-white">{`${item?.city}, ${item?.state}`}<p className="text-[11px] text-[#919191] mt-[5px]">{CountryDATAEnum[item?.country]}</p>
                                                        </td>
                                                        <td onClick={() => { setCurrentItem(item); setModal(true) }} colSpan={2} className="text-[15px] w-[22%] bg-white ">
                                                            <div className="grid gap-[8px]">
                                                                <ProgressBar Progress=
                                                                    {
                                                                        statusPercentages[item?.status]
                                                                    }
                                                                />
                                                                <div className="text-[11px] text-[#919191] mt-[5px]">Status: {statusEnum[item?.status]}</div>
                                                            </div>
                                                        </td>
                                                        {/* <div ref={dropdownRef} className={`${interview === item?.id ? '' : 'hidden'} -py-[6px] mt-[5px] -px-[8px] border text-center min-h-[120px] min-w-[100px] bg-[white] rounded-[6px] absolute bottom-[-100px] left-[-50px] -border z-[50] -left-[50%]`}> */}

                                                        <td className="relative rounded-lg w-[22%] bg-white ">
                                                            <div
                                                                onClick={() => { if (item?.status === 'offer' || item?.status === 'interview' || item?.contactRequest?.isRequested) setInterview(item?.id); }}
                                                                className={` relative h-[30px] w-[30px] ${!(item?.status === 'offer' || item?.status === 'interview' || item?.contactRequest?.isRequested) ? 'cursor-not-allowed' : 'cursor-pointer'}`}
                                                            >
                                                                <img className="" src="/assets/images/job-listing-three-dot.svg" alt="" />
                                                                { (item?.status === 'offer' || item?.status === 'interview' || item?.contactRequest?.isRequested) &&
                                                                    ((item?.interviewDetails?.seekerResponse?.status === "pending" && item?.status === 'interview') || (item?.contactRequest?.isRequested && item?.contactRequest?.status === "pending")) &&  (
                                                                    <div className="min-h-[6px]  absolute top-[0px] right-[3px] min-w-[6px] bg-[red] rounded-full"></div>
                                                                )}
                                                            </div>
                                                            <div ref={dropdownRef} className={`${interview === item?.id ? '' : 'hidden'} mt-[5px] border text-center min-w-[100px] bg-white rounded-[6px] absolute top-[50px] left-1/3 transform -translate-x-1/2 z-[50]`}>
                                                                {item?.status === 'offer' && (
                                                                    <p className="text-nowrap py-[10px] px-[20px] border-b hover:bg-[#F4F8FB]">OFFER</p>
                                                                )}
                                                                {item?.status === 'interview' && (
                                                                    <p onClick={() => { interviewClicked(index, item) }} className="text-nowrap py-[8px] px-[20px] hover:bg-[#F4F8FB]">INTERVIEW</p>
                                                                )}
                                                                {item?.contactRequest?.isRequested && (
                                                                    <p onClick={() => { contactReaquest(item) }} className="text-nowrap py-[8px] px-[20px] hover:bg-[#F4F8FB]">CONTACT REQUEST</p>
                                                                )}
                                                                {console.log(item?.contactRequest)}
                                                            </div>
                                                        </td>
                                                    </tr>

                                                    <tr className="border-0">
                                                        <td colSpan={8} className="transparent h-[10px] p-0"></td>
                                                    </tr>
                                                </>
                                            ))}
                                        </tbody>
                                    </table> :
                                    <div className="job-listing-table-box  flex w-full flex justify-center items-center">
                                        <span>No record found</span>
                                    </div>
                                }
                            </div>
                            <div className="pagination-wrapper pt-[50px] relative flex flex-row items-center justify-between -mt-6  w-[100%]">
                                {jobBoardData?.jobs && jobBoardData?.jobs?.length > 0 && <>
                                    <div>
                                        <span>
            ...
            <JobPreviewModel modal={modal} setModal={setModal} currentItem={currentItem} />
        </>
    )
}
